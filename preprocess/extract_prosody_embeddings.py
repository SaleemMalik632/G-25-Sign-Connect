...
def prepare_data(hparams, stft):
    arpabet_dict = cmudict.CMUDict('E:\\FYp R And D\\implemetation - Copy\\preprocess\\data\\cmu_dictionary\\cmudict-0 (4).7b')
    ...
    for audio_path, text, speaker_id in dataloader.audiopaths_and_text:
        text_encoded.append(torch.LongTensor(text_to_sequence(text, hparams.text_cleaners, arpabet_dict))[None, :])
        mel.append(load_mel(audio_path, hparams, stft))
        pitch_contour.append(dataloader[idx][3][None])
...
stft = TacotronSTFT(hparams.filter_length, hparams.hop_length, hparams.win_length,
                    ...
                    hparams.mel_fmax)
# mellotron = load_model(hparams).eval()
# mellotron.load_state_dict(torch.load("mellotron_libritts.pt", map_location=torch.device('cpu'))['state_dict'])
