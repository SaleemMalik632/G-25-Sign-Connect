...
const SocialLoginOptions: React.FC = () => {
    const navigate = useNavigate(); // Correctly initializing the navigate function
    const handleGoogleLoginSuccess = (credentialResponse: any) => {
        try {
            const decodedToken: DecodedToken = jwtDecode(credentialResponse.credential);
            const userName = decodedToken.name || "User";
            
            // Save the userName in localStorage (or state management)
            ...
            alert("Login successful, but unable to retrieve user information.");
        }
    };
